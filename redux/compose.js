 ...
 *  f0  f1  f2
 *
 *  上述compose(f0, f1, f2)(next)效果与下面等同 ( f = compose(f0, f1, f2) )
 *  f(next) --> f0(f1(f2(next)));
 *  f(next)返回值为如下函数
 *  n => { console.log('f0'); n = next(n); return n + 1; } (f0  next = f1(next))
 *          (上面next值为函数)--> n => { console.log('f1'); n = next(n); return n * 2; }  (f1  next = f2(next))
 *                                        (上面next值为函数)--> n => { console.log('f2'); n = next(n); return n * n; } (f2  next = n => n)
 *                                                                      (上面next值为函数)--> n => n
 *  所以fn = f(next)，调用fn(1)时，代码是从第一个函数开始运行的，然后到next的时候逐层往下，直到最后传入的next，再逐层返回，与递归同
